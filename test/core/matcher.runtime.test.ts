...
test("list: element match propagates", () => {
    const T = list(integer(0, 15), {maxLength: 8})
    const m = matchType(T, pList(pInteger(0, 255)))
    assert.equal(m?.kind, SemanticTypeKinds.List)
    assert.equal(m?.kind === SemanticTypeKinds.List && m.maxLength, 8)
    assert.equal(
...
test("list: a maxLength outside the pattern's fails", () => {
    assert.equal(matchType(list(integer(0, 15), {maxLength: 1024}), pList(pInteger(0, 255), {maxLength: 256})), undefined)
    assert.equal(matchType(list(integer(0, 15)), pList(pInteger(0, 255), {maxLength: 256})), undefined)
})

test("list: a minLength outside the pattern's fails", () => {
    assert.equal(matchType(list(integer(0, 15), {minLength: 2}), pList(pInteger(0, 255), {minLength: 4})), undefined)
    assert.notEqual(matchType(list(integer(0, 15), {minLength: 6, maxLength: 6}), pList(pInteger(0, 255), {minLength: 4, maxLength: 8})), undefined)
})
