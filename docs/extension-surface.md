...
**Status: decided, and implemented in `src/codecs/engine/scope.ts`.** What
survives here is the reasoning: where an extension's DSL ergonomics live,
and what it would cost to put them anywhere else. The carriers themselves
are code now — a rule body receives its procedure's `CodecScope` as
`produce`'s fourth argument — so this file no longer sketches them.
...
hand-picked constants in fragment text — `binary-rules.ts` carried its own
...
## 5. What the TS side gets that the DSL side cannot
